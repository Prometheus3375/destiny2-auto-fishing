...
if __name__ == '__main__':
    def main():
        localizations_str = ', '.join(name.split('_', 1)[0] for name in available_localizations)
        keys_str = ', '.join(name.split('_', 1)[0] for name in available_keys)
        ...
        parser.add_argument(
            ...
            help='the localization you use in Destiny 2; case insensitive. '
                 f'Supported localizations: {localizations_str}',
            )
        parser.add_argument(
            'interact_key',
            help='the interaction key you use in Destiny 2; case insensitive. '
                 f'Supported keys: {keys_str}',
            )
        parser.add_argument(
            'fish_limit',
            nargs='?',
